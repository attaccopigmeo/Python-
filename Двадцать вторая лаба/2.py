...
def draw_adjacency_matrix(adj_matrix):
    ...
    # Подписываем вершины
    for i in range(n):
        ax.text(x[i], y[i], str(i + 1), ha='center', va='center', fontsize=12)
...
# Используем алгоритм поиска в ширинAу
...
queue = [(k - 1, -1)] # в очереди храним пары (номер города с 0, расстояние от K)
visited = [False] * n
ans = []
while len(queue) > 0:
    cur, dist = queue[0]
